                          ...
                          const char *nmemb, int psec)
{
    ...
    const struct wheel *pool = &B->perm_pool;
    int n = pool->n;
    // Incremental odometer for a pure UNORDERED choice (r == j, no pre/post
    // wheels): decode a lane's run start once (colex unrank), then step r by +1
    // with the O(k) next-combination-in-colex successor -- no per-candidate
    // binary search over the __global Pascal table. Ordered arrangements need a
    // k-permutation successor (an NP-wide in-use scan) and stay per-candidate;
    // with surrounding wheels the faster post wheels already amortise the unrank.
    int perm_inc = (nw == 0 && !ord);
    ...
    fputs(")\n{\n", ms);
    if (perm_inc) {
        // Incremental (r == j; NALL is the tile's exclusive end). Decode the run
        // start once, then step colex: within a size, the next combination is an
        // O(k) carry; step to the next size only when the size block wraps.
        fputs("#if RUN > 1\n"
        ...
        fputs("    int s = LO; ulong rr = r0;\n"
              "    for (;;) { ulong blk = PSZ[s]; if (rr < blk) break; rr -= blk; s++; }\n"
              "    { ulong jj = rr; int up = NP;\n"
              "      for (int k = s; k >= 1; k--) {\n"
              "          int lo2 = k - 1, hi2 = up - 1;\n"
              "          while (lo2 < hi2) { int mid = (lo2 + hi2 + 1) >> 1;\n"
              ...
              "    for (ulong r = r0;;) {\n"
              "        int p = 0;\n"
              "        for (int pp = 0; pp < s; pp++) { int it = idx[pp];\n", ms);
        if (pool->L == 1)      fputs("            buf[p++] = PB[it];\n", ms);
        else if (pool->L > 1)  fprintf(ms, "            for (int t = 0; t < %d; t++) buf[p++] = PB[it*%d + t];\n", pool->L, pool->L);
        else                   fputs("            { int o = PO[it], l = PL[it]; for (int t = 0; t < l; t++) buf[p++] = PB[o + t]; }\n", ms);
        fputs("        }\n", ms);
        ...
        fprintf(ms,
              ...
              "        if (++r >= rend) break;\n"
              "        if (++rr == PSZ[s]) { s++; rr = 0; for (int q = 0; q < s; q++) idx[q] = q; }\n"
              "        else { int i = 0; while (i < s && idx[i] + 1 == (i + 1 < s ? idx[i+1] : NP)) i++;\n"
              "               idx[i]++; for (int q = 0; q < i; q++) idx[q] = q; }\n"
              "    }\n"
              "#else\n", HA->dglen, HA->gpu_fn);
    }

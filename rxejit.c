...
// The -G backend. Where the CPU path steps an odometer over a shard, the GPU
// gives one lane per index: each work-item unranks its global id into a
// candidate -- the very seed the CPU emits, minus the stepping -- hashes it,
// and looks its digest up in the sorted target set, appending the rare hit.
// A grid launch over a tile of [0,N) is the shard split, now in silicon; this
// is the limit of "shard from the beginning". rxejit generates a host C program
// with the OpenCL kernel embedded as a string (the device runtime RXEJIT_CL
// plus this mask's __constant alphabets and crack()), builds it at runtime with
// clBuildProgram -- the shell-out-to-cc trick one level down -- launches tiles,
// and reconstructs each hit's plaintext on the host to print <digest>:<plain>,
// the same line the CPU keycrack prints. Fixed-width masks only (no divergence).
static void emit_gpu(FILE *o, const char *pattern, const struct build *B, const char *nmemb)
{
    struct wheel *w = B->w;
    int nw = B->nw;
    int off[MAXW], TL = 0;
    for (int i = 0; i < nw; i++) { off[i] = TL; TL += w[i].L; }

    // Build the full kernel source -- device runtime then this mask's part --
    // into a buffer, so it goes into the host program as one escaped C string.
    ...
    for (int i = 0; i < nw; i++) {
        ...
        for (int j = 0; j < w[i].n * w[i].L; j++)
            fprintf(ms, "%s%d", j ? "," : "", (unsigned char)w[i].base[j]);
        fputs("};\n", ms);
    }
    fputs("__kernel void crack(ulong base, ulong N, __global const uchar *tgt, uint ntgt,\n"
          "                    __global ulong *hits, volatile __global uint *nhits)\n{\n"
          "    ulong idx = base + (ulong)get_global_id(0);\n"
          "    if (idx >= N) return;\n", ms);
    ...
    for (int i = 0; i < nw; i++) {
        if (w[i].L == 1) fprintf(ms, "    buf[%d] = A%d[d%d];\n", off[i], i, i);
        else for (int j = 0; j < w[i].L; j++)
            fprintf(ms, "    buf[%d] = A%d[d%d*%d + %d];\n", off[i] + j, i, i, w[i].L, j);
    }
    fprintf(ms, "    uchar dg[16]; cl_md5(buf, %d, dg);\n", TL);
    fputs("    if (cl_tgt_has(tgt, ntgt, dg)) { uint s = atomic_inc(nhits); if (s < (1<<20)) hits[s] = idx; }\n"
          "}\n", ms);
    fclose(ms);
    ...
    fputs(" */\n#define CL_TARGET_OPENCL_VERSION 300\n"
          "#include <stdio.h>\n#include <stdlib.h>\n#include <string.h>\n#include <CL/cl.h>\n\n", o);
    fputs(RXEJIT_RT, o); fputc('\n', o);       // rt_md5, to re-hash a hit on the host
    ...
    // Reconstruct member 'idx' on the host -- the same unrank, in C -- to print
    // the plaintext of a hit and re-derive its digest.
    fputs("static void render(unsigned long long idx, unsigned char *buf)\n{\n", o);
    for (int i = 0; i < nw; i++) {
        fprintf(o, "    static const unsigned char A%d[] = {", i);
        for (int j = 0; j < w[i].n * w[i].L; j++)
            fprintf(o, "%s%d", j ? "," : "", (unsigned char)w[i].base[j]);
    ...
    fputs("    unsigned long long f = idx;\n", o);
    for (int i = nw - 1; i >= 0; i--)
        fprintf(o, "    unsigned d%d = f %% %d; f /= %d;\n", i, w[i].n, w[i].n);
    for (int i = 0; i < nw; i++) {
        if (w[i].L == 1) fprintf(o, "    buf[%d] = A%d[d%d];\n", off[i], i, i);
        else for (int j = 0; j < w[i].L; j++)
            fprintf(o, "    buf[%d] = A%d[d%d*%d + %d];\n", off[i] + j, i, i, w[i].L, j);
    }
    fputs("}\n\n", o);

    // The targets: MD5 hex digests, one per line, decoded to 16 bytes and sorted
    // so the device can binary-search them.
    ...
    fputs("int main(int argc, char **argv)\n{\n"
          ...
          "    cl_kernel k = clCreateKernel(pr, \"crack\", &e); CK(e);\n\n"
          "    cl_ulong N = NMEMB;\n"
          "    cl_mem mt = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, (size_t)ntgt * 16, tgt, &e); CK(e);\n"
          "    cl_ulong *hits = calloc(MAXHITS, sizeof *hits);\n"
          "    cl_uint nhits = 0;\n"
          "    cl_mem mh = clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, (size_t)MAXHITS * sizeof(cl_ulong), NULL, &e); CK(e);\n"
          "    cl_mem mn = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof nhits, &nhits, &e); CK(e);\n"
          "    CK(clSetKernelArg(k, 1, sizeof N, &N));\n"
          "    CK(clSetKernelArg(k, 2, sizeof mt, &mt));\n"
          "    CK(clSetKernelArg(k, 3, sizeof ntgt, &ntgt));\n"
          "    CK(clSetKernelArg(k, 4, sizeof mh, &mh));\n"
          "    CK(clSetKernelArg(k, 5, sizeof mn, &mn));\n\n"
          "    cl_ulong TILE = 1ULL << 24;\n"
          "    for (cl_ulong base = 0; base < N; base += TILE) {\n"
          "        cl_ulong n = (N - base < TILE) ? (N - base) : TILE;\n"
          ...
          "    CK(clEnqueueReadBuffer(q, mh, CL_TRUE, 0, (size_t)got * sizeof(cl_ulong), hits, 0, NULL, NULL));\n\n"
          "    for (unsigned i = 0; i < got; i++) {\n"
          "        unsigned char buf[MASKLEN], dg[16];\n"
          "        render((unsigned long long)hits[i], buf);\n"
          "        rt_md5(buf, MASKLEN, dg);\n"
          "        for (int h = 0; h < 16; h++) printf(\"%02x\", dg[h]);\n"
          "        putchar(':'); fwrite(buf, 1, MASKLEN, stdout); putchar('\\n');\n"
          "    }\n"
                           ...
                           int gpu)
{
    ...
    if (gpu) emit_gpu(f, pattern, B, nmemb);
...
"    -D dir   also look in 'dir' for a [:name:] dictionary's name.dict file.\n",
    ...
    if (nw < 0) {
        ...
        int maskw = 0;
        if (gpu) {
            if (b.lr_active || b.has_backref) gpu_no = "a variable-length structure";
            else for (int i = 0; i < nw; i++) {
                if (b.w[i].L == 0) { gpu_no = "a variable-width position"; break; }
                maskw += b.w[i].L;
            }
            if (!gpu_no && !nmemb)     gpu_no = "more members than fit 64 bits";
            if (!gpu_no && maskw >= 56) gpu_no = "a candidate wider than one MD5 block";
        }

        if (gpu && gpu_no) {
            fprintf(stderr, "%s: the GPU path needs a fixed-width mask -- this has %s.\n", prog, gpu_no);
            ret = 1;
        } else if (emit_only && gpu) { emit_gpu(stdout, pattern, &b, nmemb); ret = 0; }
        else if (emit_only) { emit(stdout, pattern, &b, sink, nmemb, verbose, hash, psec); ret = 0; }
        else if (gpu)       ret = compile_and_run(pattern, &b, sink, nmemb, NULL, matchfile, verbose, hash, psec, 1);

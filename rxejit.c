...
static void emit(FILE *o, const char *pattern, struct wheel *w, int nw, int sink)
{
    ...
    if (sink == SINK_COUNT)
        fputs("static unsigned long long total;\n\n", o);

    fputs("static void run(unsigned long long from, unsigned long long count)\n{\n", o);
    ...
    fputs("    for (;;) {\n", o);
    fputs(sink == SINK_COUNT
              ? "        total++;\n"
              : "", o);
    if (sink == SINK_WRITE)
        fprintf(o, "        fwrite(buf, 1, %d, stdout);\n", nw + 1);
    fputs("        if (count && ++done == count) return;\n", o);
    for (int i = nw - 1; i >= 0; i--)
...
// code.
static int compile_and_run(const char *pattern, struct wheel *w, int nw, int sink)
{
    ...
    emit(f, pattern, w, nw, sink);
    fclose(f);

    const char *cc = getenv("CC");
    if (!cc || !*cc) cc = "cc";
    char *cargv[] = { (char *)cc, "-O2", src, "-o", exe, NULL };
    int rc = spawn(cargv);
    if (rc != 0) {
...
"    -n  count the members rather than print them (times the walk without I/O).\n",
                    prog);
                return opt == 'h' ? 0 : 2;
        }
    }
    if (optind != argc - 1) {
        fprintf(stderr, "usage: %s [-S] [-n] REGEX\n", prog);
        return 2;
    }
    ...
    if (nw < 0) {
        ...
        emit(stdout, pattern, w, nw, sink);
        ret = 0;
    } else {
        ret = compile_and_run(pattern, w, nw, sink);
    }

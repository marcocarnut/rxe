                           ...
                           int gpu)
{
    ...
    if (gpu == 2)   emit_gpu_hybrid(f, pattern, B, psec);
...
"    -D dir   also look in 'dir' for a [:name:] dictionary's name.dict file.\n",
    ...
    if (nw < 0) {
        ...
        int gpu_kind = 0;                       // 1 = fixed mask, 2 = loop repeat
        int maskw = 0;
        if (gpu && b.lr_active) {
            ...
            if (b.has_backref) gpu_no = "a variable-length structure";

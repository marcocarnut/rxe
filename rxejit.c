...
// gpu_w0, when set, is a GPU hash returning only the digest's first 32-bit word
// (big-endian): the MD-family (md5/ntlm) rotate their state, so the first word
// is finalized a few rounds early and the compiler drops the rest -- the GPU
// then compares one word and the host re-hashes the rare hits to confirm. SHA
// finalizes its first word only at the last round, so it has no such variant.
struct hashalg { const char *name; int dglen; int mfac; const char *cpu_fn, *gpu_fn, *gpu_w0; };
static const struct hashalg HASHES[] = {
    { "md5",    16, 1, "rt_md5",    "cl_md5",    "cl_md5_w0"  },
    { "ntlm",   16, 2, "rt_ntlm",   "cl_ntlm",   "cl_ntlm_w0" },  // MD4(UTF-16LE): message 2x
    { "sha1",   20, 1, "rt_sha1",   "cl_sha1",   NULL          },
    { "sha256", 32, 1, "rt_sha256", "cl_sha256", NULL          },
};
...
static void emit_cl_check(FILE *ms, const char *len);   // defined with the other kernel helpers
static void emit_gpu_hybrid(FILE *o, const char *pattern, const struct build *B, int psec)
{
    ...
    fputs("static int cmpdg(const void *a, const void *b) { return memcmp(a, b, DG); }\n"
          "static int tgt_has(const unsigned char *t, unsigned ntgt, const unsigned char *dg) {\n"
          "    int lo = 0, hi = (int)ntgt - 1;\n"
          "    while (lo <= hi) { int mid = (lo + hi) >> 1;\n"
          "        int c = memcmp(dg, t + (size_t)mid * DG, DG);\n"
          "        if (c == 0) return 1; if (c < 0) hi = mid - 1; else lo = mid + 1; }\n"
          "    return 0;\n}\n"
    ...
    fputs("int main(int argc, char **argv)\n{\n"
          ...
          "        HASHFN(p, hlen[i], dg);\n"
          "        if (!tgt_has(tgt, ntgt, dg)) continue;   /* a first-word false positive */\n"
          "        confirmed++;\n"
...
// Emit a lane's hash-and-target-check into a kernel, up to and including the
// "if (...) " -- the caller appends the { record } block. For an MD-family hash
// (gpu_w0 set) it hashes to the first word and probes cl_tgt_has_w0, an early
// reject the host confirms; otherwise the full digest and cl_tgt_has. 'len' is
// the candidate-length expression (T or p).
static void emit_cl_check(FILE *ms, const char *len)
{
    if (HA->gpu_w0)
        fprintf(ms, "    uint w0 = %s(buf, %s);\n"
                    "    if (cl_tgt_has_w0(tgt, ntgt, w0)) ", HA->gpu_w0, len);
    else
        fprintf(ms, "    uchar dg[%d]; %s(buf, %s, dg);\n"
                    "    if (cl_tgt_has(tgt, ntgt, dg)) ", HA->dglen, HA->gpu_fn, len);
}
                             ...
                             int lowvar, const char *nmemb, int psec)
{
    ...
    fputs("int main(int argc, char **argv)\n{\n"
          ...
          "    int INFLIGHT = getenv(\"RXEJIT_INFLIGHT\") ? atoi(getenv(\"RXEJIT_INFLIGHT\")) : 2;\n"
          "    if (INFLIGHT < 1) INFLIGHT = 1; if (INFLIGHT > 16) INFLIGHT = 16;\n"
          ...
          "        /* Up to INFLIGHT tiles in flight (RXEJIT_INFLIGHT, default 2):\n"
          "           enqueue several before waiting on the oldest, so a short kernel\n"
          "           does not starve the GPU on the host between launches. Fully\n"
          "           drained before the next prefix reuses the shared pfx buffer. */\n"
          "        cl_event evr[16] = {0}; cl_ulong evn[16] = {0}; int rs = 0;\n"
          "        for (cl_ulong base = 0; base < loN; base += TILE) {\n"
          "            cl_ulong n = (loN - base < TILE) ? (loN - base) : TILE;\n"
          "            size_t global = (size_t)((n + 255) / 256) * 256;\n"
          "            CK(clSetKernelArg(k, 0, sizeof base, &base));\n"
          "            cl_event ev;\n"
          "            CK(clEnqueueNDRangeKernel(q, k, 1, NULL, &global, NULL, 0, NULL, &ev));\n"
          "            if (evr[rs]) { CK(clWaitForEvents(1, &evr[rs])); DRAIN(evr[rs], evn[rs]); clReleaseEvent(evr[rs]); }\n"
          "            evr[rs] = ev; evn[rs] = n; rs = (rs + 1) % INFLIGHT;\n"
          "        }\n"
          "        for (int di = 0; di < INFLIGHT; di++) {\n"
          "            if (evr[rs]) { CK(clWaitForEvents(1, &evr[rs])); DRAIN(evr[rs], evn[rs]); clReleaseEvent(evr[rs]); evr[rs] = 0; }\n"
          "            rs = (rs + 1) % INFLIGHT;\n"
          "        }\n"
          "    }\n"
          "    fclose(pf);\n"

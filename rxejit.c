                 ...
                 int sink, const char *nmemb)
{
    ...
    int threaded = count && nmemb;              // match threads in a later step
    ...
    if (match) {
        fputs(RXEJIT_RT, o);
        fputs("\nstatic struct rt_set TB;   /* the targets, read-only once loaded */\n\n", o);
    }
...
"}\n", nmemb);
        ...
        // Load the targets, sift the whole set through them, print the hit
        // total to stderr so it does not mix with the matched members on stdout.
                           ...
                           const char *matchfile)
{
    ...
    // The match program takes the target file as argv[1]; the count program
    // takes its thread count there (forwarded from -j). Write takes neither.
    char *rargv[] = { exe, (char *)(matchfile ? matchfile : jobs), NULL };
    if (!matchfile && !jobs) rargv[1] = NULL;
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "Snj:m:h")) != -1) {
        switch (opt) {
            ...
            default:
                fprintf(stderr,
"usage: %s [-S] [-n | -m file] [-j jobs] REGEX\n"
...
"             single-threaded and ordered.\n",
    ...
    if (optind != argc - 1) {
        fprintf(stderr, "usage: %s [-S] [-n | -m file] [-j jobs] REGEX\n", prog);
        return 2;
    }

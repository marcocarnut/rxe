...
"  uint4* t  = smem + (u32)(BWORDS4+YO4)*S + threadIdx.x;\n"
...
"    for(u32 q=0;q<BWORDS4;q++) t[q*S]=gV[(s*BWORDS4+q)*NC+tid];\n"
"    for(u32 st=0;st<steps;st++) blockmix_sh(t,yo,S);\n"
"    for(u32 q=0;q<BWORDS4;q++){ uint4 tv=t[q*S],bv=b[q*S]; b[q*S]=make_uint4(bv.x^tv.x,bv.y^tv.y,bv.z^tv.z,bv.w^tv.w); }\n"
"    blockmix_sh(b,yo,S);\n"
"  }\n"
...
int main(int argc,char**argv){
  ...
  unsigned shbytes=(64u+32u+64u)*TPB*16u;   /* (BWORDS4+YO4) uint4 * TPB */
